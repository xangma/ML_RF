        ...
        for k in range(0,probshape):
            for j in range(len(filtstats)):
                ...
                for i in range(filtstats[j][0]): # Plot for all filtersrpr
                    ...
                    plt.xlim(5,32)
#                plt.show()
                outname='%sbandvprob%s_allfilt_%s.png' %(fullsavedir,names[k],j)
                ...
                for i in range(filtstats[j][0]): # Plot for all filters
                    ...
                    outname='%sbandvprob%s_filt_%s.png' %(fullsavedir,names[k],settings.filters[j][i])
                    outnamelist.append(outname)
                    plt.tight_layout()
                    plt.savefig(outname)
                    plt.close(fig)
...
#                plt.show()
                plt.tight_layout()
                outname='%scolourvprob%s_allfilt_%s.png' %(fullsavedir,names[k],j)
                outnamelist.append(outname)
                plt.savefig(outname)
                plt.close(fig)
                ...
                for i in range(filtstats[j][1]): # Plot for all filters
                    ...
                    plt.xlim(-10,10)
                    outname='%scolourvprob%s_filt_%s_%s_%s-%s.png' %(fullsavedir,names[k],j,i,settings.filters[j][combs[j][i][0]],settings.filters[j][combs[j][i][1]])
                    outnamelist.append(outname)
                    plt.tight_layout()
                    plt.savefig(outname)
                    plt.close(fig)
