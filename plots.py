        ...
        for k in range(0,probshape):
            for j in range(len(filtstats)):
                ...
                for i in range(filtstats[j][0]): # Plot for all filtersrpr
                    ...
                    plt.xlim(5,32)
#                plt.show()
                outname='plots/bandvprob%s_allfilt_%s.png' %(names[k],j)
                ...
                for i in range(filtstats[j][0]): # Plot for all filters
                    ...
                    outname='plots/bandvprob%s_filt_%s.png' %(names[k],settings.filters[j][i])
                    outnamelist.append(outname)
                    plt.tight_layout()
                    plt.savefig(outname)
                    plt.close(fig)
...
#                plt.show()
                plt.tight_layout()
                outname='plots/colourvprob%s_allfilt_%s.png' %(names[k],j)
                outnamelist.append(outname)
                plt.savefig(outname)
                plt.close(fig)
                ...
                for i in range(filtstats[j][1]): # Plot for all filters
                    ...
                    plt.xlim(-10,10)
                    outname='plots/colourvprob%s_filt_%s_%s_%s-%s.png' %(names[k],j,i,settings.filters[j][combs[j][i][0]],settings.filters[j][combs[j][i][1]])
                    outnamelist.append(outname)
                    plt.tight_layout()
                    plt.savefig(outname)
                    plt.close(fig)
