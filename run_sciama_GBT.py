...
runpaths=[]
runnames=[]
for nlearn in range(0,len(n_learningrate)):
    for k in range(0,len(n_estimators)):
        for j in range(0,len(n_train)):
            for i in range(0,len(n_depth)):
                run_nameopts='RUN_'+timestr+'_GBT'
                run_namewo=run_nameopts+'_n_depth_'+str(n_depth[i])+'_n_tr_'+str(n_train[j])+'_n_est_'+str(n_estimators[k])+'_learnrate_'+str(n_learningrate[nlearn]) 
                run_name = '/'+run_namewo+'/'
                ...
                with open(fullpath+'settings.py',"a") as set_file: 
                    ...
                    set_file.write("\n")
                    set_file.write("\nMLA = 'sklearn.ensemble.GradientBoostingClassifier'")
                    set_file.write("\nMLAset = {'n_estimators': %s,'verbose':True,'max_depth':%s,'learning_rate':%s}"%(n_estimators[k],n_depth[i],n_learningrate[nlearn]))
