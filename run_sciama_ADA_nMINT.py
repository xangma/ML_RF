...
n_estimators=numpy.array([8,16,32,64,128,256,512,1024,2048,4096,8192])
#n_estimators=numpy.array([16])
n_train=numpy.array([100,500,1000,2500,5000,10000,25000])
n_learningrate=numpy.array([0.80,0.84,0.88,0.92,0.96,1.00,1.04,1.08,1.12,1.16,1.20])
...
numpy.savez("stats_%s" %('RUN_'+timestr),n_estimators=n_estimators,n_learningrate=n_learningrate,n_train=n_train,MINT_n_feat=MINT_n_feat)#filters=filters,use_colours=use_colours)
...
for nmint in range(0,len(MINT_n_feat)):
    for k in range(0,len(n_estimators)):
        for j in range(0,len(n_train)):
            for i in range(0,len(n_learningrate)):
                run_nameopts='RUN_'+timestr+'_ADA_nMINT'
                run_namewo=run_nameopts+'_n_learningrate_'+str(n_learningrate[i])+'_n_tr_'+str(n_train[j])+'_n_est_'+str(n_estimators[k])+'_nMINT_'+str(MINT_n_feat[nmint])#+'max_feat_None' 
                run_name = '/'+run_namewo+'/'
    ...
    #MLAset = 
                    ...
                    set_file.write("\nMLAset = {'n_estimators':%s,'learning_rate':%s}"%(n_estimators[k],n_learningrate[i]))
