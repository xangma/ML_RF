...
#    print(unique_n_est[i])
    ...
    plt.figure()
    for j in range(len(sort))[3:-2]:
        plt.plot(numpy.array(range(len(feat_arr[0]))),feat_sub_arr[n_est_ind_sorted[j]],label='n_train == %s' %n_train_arr[mask][sort][j])
    plt.axvspan(5, 15, color='red', alpha=0.3)
    plt.axvspan(20, 30, color='red', alpha=0.3)
    plt.axvspan(35, 45, color='red', alpha=0.3)
    plt.legend()
...
#    print(unique_n_train[i])
    ...
    for j in range(len(sort))[5:-2]:
        plt.plot(numpy.array(range(len(feat_arr[0]))),feat_sub_arr[n_train_ind_sorted[j]],label='n_est == %s' %n_estimators_arr[mask][sort][j])
    plt.axvspan(5, 15, color='red', alpha=0.3)
    plt.axvspan(20, 30, color='red', alpha=0.3)
    plt.axvspan(35, 45, color='red', alpha=0.3)
    plt.legend()
